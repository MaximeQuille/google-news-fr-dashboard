...
create table if not exists public.dashboard_stats (
  ...
  first_article timestamp without time zone,
  last_article timestamp without time zone,
  updated_at timestamp with time zone default now(),
  constraint dashboard_stats_singleton check (id = 'main')
);
...
drop policy if exists "Public read dashboard stats" on public.dashboard_stats;
create policy "Public read dashboard stats"
...
grant select, insert, update, delete on public.dashboard_stats to service_role;
...
create table if not exists public.alert_email_outbox (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default now(),
  sent_at timestamp with time zone,
  status text not null default 'pending',
  error text,
  filter_id uuid references public.alert_filters(id) on delete set null,
  email text not null,
...
create index if not exists idx_alert_email_outbox_pending on public.alert_email_outbox(status, created_at) where sent_at is null;

alter table public.alert_filters enable row level security;
alter table public.alert_deliveries enable row level security;
alter table public.alert_email_outbox enable row level security;
...
grant select, insert, update, delete on public.alert_email_outbox to service_role;
